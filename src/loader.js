...
function ngcLoader() {
    ...
    var timeLabel = "ngcLoader+" + sourceFileName + "+";
    if (!cb) {
    ...
    plugin.done
        ...
        if (sourceFileName.endsWith('.ts')) {
            result.errorDependencies.forEach(function (dep) { return _this.addDependency(dep); });
        ...
        if (ngFactoryRe.test(sourceFileName)) {
            ...
            origDependencies.forEach(function (dep) { return _this.addDependency(dep); });
        }
        ...
        if (ngStyleRe.test(sourceFileName)) {
            ...
            var styleDependencies = plugin.getResourceDependencies(styleFile);
            styleDependencies.forEach(function (dep) { return _this.addDependency(dep); });
        }
